		...
		struct lvsVirtualServer *eserver)
{
	...
	int lvsParseConfig (int fd, struct lvsConfig *config, int *badLineNum)
	{
		...
		*badLineNum = 0;
		...
		if (!sb.st_size)
		{
			//debug_print(INFOAGENT_LOG_FILE,DEBUG_ERROR,"Lvs read size error.");
		...
		{
			//debug_print(INFOAGENT_LOG_FILE,DEBUG_ERROR,"Lvs read config file contents, size matching error. size read using fstat %d", sb.st_size);
		...
		if (rc)
		{
			//debug_print(INFOAGENT_LOG_FILE,DEBUG_ERROR,"parseConfiguration failed with error %d", rc);
			*badLineNum = configFile.lineNum;
		}

		//debug_print(INFOAGENT_LOG_FILE,DEBUG_ERROR,"parseConfiguration returning success.");
		return rc;
	}
